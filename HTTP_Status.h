...
public:
    ...
    explicit HTTP_Status(const StatusCode code = OK): code(code) {};
    HTTP_Status& operator=(const StatusCode statusCode) { code = statusCode; return *this;};
    HTTP_Status& operator=(const HTTP_Status& status) { code = status.code; return *this;};

    [[nodiscard]] string extract() const noexcept {
        return to_string(code) + " " + StatusText.at(code);
    }
